...
namespace LOSA.TransaccionesPT
{
    public partial class frmdespacho_tipo_detalle_carga : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            string query = @"sp_get_presentacion_sacos";
            SqlConnection conn = new SqlConnection(dp.ConnectionStringLOSA);
            conn.Open();
            SqlCommand cmd = new SqlCommand(query, conn);
            cmd.CommandType = CommandType.StoredProcedure;
            SqlDataAdapter adat = new SqlDataAdapter(cmd);
            dsProductos.presentacion_sacos.Clear();
            adat.Fill(dsProductos.presentacion_sacos);
        }
        ...
        private void cmdImprimir_Click(object sender, EventArgs e)
        {
            ...
            {
                //Reporte con Filas
                LOSA.Despachos.Reportes.frm_despacho_con_filas rpt = new LOSA.Despachos.Reportes.frm_despacho_con_filas(id_despacho, Convert.ToInt32(grdDestinos.EditValue), Convert.ToInt32(grdPresentacion.EditValue));
